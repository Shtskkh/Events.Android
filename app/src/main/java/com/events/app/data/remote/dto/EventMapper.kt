...
fun ShortEventDto.toDomain(): Event {
    ...
    return Event(
        ...
        description = "", startDate = parseDateTime(startDateTime), endDate = parsedEnd,
        format = format ?: "", places = 0, location = locationTitle ?: "",
        placeNumber = null, link = null, type = type ?: "", isPublic = true,
        isFinished = LocalDateTime.now().isAfter(parsedEnd),
        previewUrl = buildPreviewUrl(previewInfo), needsRegistration = false,
        maxParticipants = null, organizerName = null,
        userId = null, participantsCount = null, viewsCount = null,
    )
}
...
fun EventDetailDto.toDomain(): Event {
    ...
    return Event(
        ...
        startDate = parseDateTime(startDateTime), endDate = parsedEnd,
        format = format ?: "", places = maxParticipants ?: 0,
        location = locationTitle ?: "", placeNumber = placeInfo?.number,
        link = null, isPublic = isPublic ?: true,
        isFinished = LocalDateTime.now().isAfter(parsedEnd),
        previewUrl = buildPreviewUrl(previewInfo),
        needsRegistration = needsRegistration ?: false,
        maxParticipants = maxParticipants, organizerName = organizerName,
        userId = userId,
